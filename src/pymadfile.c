...
static void py_madfile_dealloc(PyObject * self, PyObject * args) {
	if (PY_MADFILE(self)->f) {
		...
		/* for some reason, this function is segfaulting python when it cleans
		 * up */
...
static unsigned short madfixed_to_ushort(mad_fixed_t f) {
	 ...
	 *
	 * The unsigned short value is formed by the least significant whole
	 * part bit, followed by the 15 most significant fractional part bits.
	 * Warning: this is a quick and dirty way to compute the 16 bit number,
	 * madplay includes much better algorithms. */
...
static PyObject * py_madfile_read(PyObject * self, PyObject * args) {
	...
	/* nextframe might get set during this loop, in which case the bucket
	 * needs to be refilled */
	do {
		nextframe = 0; /* reset */

	/* The input bucket must be filled if it becomes empty or if it's the
	 * first execution on this file */
	if ((MAD_STREAM(self).buffer == NULL) ||
		 ...
		 *     back at the beginning of the buffer and taken into account
		 *     before refilling the buffer.  This means that the input
		 *     buffer must be large enough to hold a whole frame at the
		 *     highest observable bitrate (currently 448kbps).
		 */
	...
	/* Decode the next mpeg frame.  The streams are read from the buffer,
	 * its constituents are broken down and stored in the frame structure,
	 * ready for examination/alteration or PCM synthesis.  Decoding options
	 * are carried to the Frame from the Stream.
	 *
	 ...
	 *
	 * [2] When a fatal error is encountered, all decoding activities shall
	 *     be stopped, except when a MAD_ERROR_BUFLEN is signalled.  This
	 *     condition means that the mad_frame_decode() function needs more
	 *     input do its work.  Once should refill the buffer and repeat the
	 *     mad_frame_decode() call.  Some bytes may be left unused at the
	 *     end of the buffer if those bytes form an incomplete frame.  Before
	 *     refilling, the remaining bytes must be moved to the beginning of
	 *     the buffer and used for input for the next mad_frame_decode()
	 *     invocation.  (See the comment marked [1] for earlier details)
	 *
	 * Recoverable errors are caused by malformed bitstreams, in this case
	 * one can call again mad_frame_decode() in order to skip the faulty
	 * part and resync to the next frame.
	 */
	...
	/* The characteristics of the streams first frame are printed on stderr.
	 * The first frame is representative of the entire stream */
#if 0
	 ...
	 * structure.  It is expressed as a fixed point number whose data type
	 * is mad_timer_t.  It is different from the fixed point format of the
	 * samples and unlike it, cannot be directly added or subtracted.  The
	 * timer module provides several functions to operate on such numbers.
	 * Be careful though, as some functions of mad's timer module receive
	...
	/* Create the buffer to store the PCM samples in, so python can use it
	 * we do 4 pointer increments per sample in the buffer, so make it 4
	 * times as big as the number of samples */
	...
	/* Synthesised samples must be converted from mad's fixed point format
	 * to the consumer format.  Here we use unsigned 16 bit big endian ints
	 * on two channels.  Integer samples are temporarily stored in a buffer
	 * that is flushed when full. */
	for (i = 0; i < MAD_SYNTH(self).pcm.length; i++) {
		unsigned short sample;

		/* left channel */
		sample = madfixed_to_ushort(MAD_SYNTH(self).pcm.samples[0][i]);
		*(buffy++) = sample & 0xFF;
		*(buffy++) = sample >> 8;
		...
		if (MAD_NCHANNELS(&MAD_FRAME(self).header) == 2)
			sample = madfixed_to_ushort(MAD_SYNTH(self).pcm.samples[1][i]);
		*(buffy++) = sample & 0xFF;
		*(buffy++) = sample >> 8;
	}
